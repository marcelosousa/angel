...
public class ProjectLanguageIndexer {
  ...
  private static class ReadRepositoryFn extends DoFn<FileIO.ReadableFile, Item> {
    @ProcessElement
    public void processElement(@Element FileIO.ReadableFile file, OutputReceiver receiver) {
      ...
      try (ReadableByteChannel byteChannel = file.open();
        ...
        if (fileCount > 0) {
          Item item = new Item(repositoryID, fileCount);
  ...
  private static void runJob(ProjectLanguageIndexerOptions options) {
    ...
    p.apply(FileIO.match().filepattern(input))
      ...
      .via(item -> item.getRepositoryName() + ": " + item.getFileCount()))
